    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
              //  .mvcMatchers(HttpMethod.GET,"/couponapi/coupons","/index","/showGetCoupon","/getCoupon","/couponDetails").hasAnyRole("USER","ADMIN")
                .mvcMatchers(HttpMethod.GET,"/viewCustomers","/viewSellers","/viewCustomeers").hasRole("ADMIN")
                .mvcMatchers(HttpMethod.GET,"/viewseller").hasRole("ADMIN")
                .mvcMatchers("/activateCustomer/**","/deactivateCustomer/**").hasRole("ADMIN")
                .mvcMatchers("/activateseller","/deactivateseller").hasRole("ADMIN")
                .mvcMatchers("/register","/login","/Customer/login","/confirm-account","/reset-password","/forgot-password").permitAll()
                .mvcMatchers("/seller/register").permitAll()
                .mvcMatchers("/userdata").permitAll()
